...
class JWTTokenProvider:
    ...
    def get_jwt_token(self):
        ...
        logging.debug("Refreshing JWT token after %s secs.", time.time() - self._token_refresh_ts)
        session = OAuth2Session(self._app_id, self._app_secret, scope="")
        token = session.fetch_token(self._token_endpoint, grant_type="client_credentials")  # authorization_code
...
class VectaraMemory(MemoryProviderSingleton):
    ...
    def __init__(self, cfg):
        self._jwt_provider = JWTTokenProvider(cfg.vectara_customer_id,
                                              cfg.vectara_app_id,
    ...
    def get_corpus_id(self, index_name):
        ...
        if response.status_code != 200:
            logger.typewriter_log(
                ...
                Fore.RED,
                Style.BRIGHT + (f"code {response.status_code}, reason {response.reason}, details {response.text}") + Style.RESET_ALL,
            )
            logger.double_check(
    ...
    def add(self, data):
        ...
        if response.status_code != 200:
            error = (f"REST upload failed with code {response.status_code}, "
                     f"reason {response.reason}, text {response.text}")
    ...
    def clear(self):
        ...
        if response.status_code != 200:
            error = (f"Reset Corpus failed with code {response.status_code}, "
                     f"reason {response.reason}, text {response.text}")
            logging.warning(error)
            return error
        return 'Obliterated'
    ...
    def get_relevant(self, data, num_relevant=5):
        ...
        if response.status_code != 200:
            logging.error("Query failed with code %d, reason %s, text %s",
                          ...
                          response.reason,
                          response.text)
            return response, False # todo check for status errors.
...
if __name__ == "__main__":
    ...
    assert len(result) == 1
    assert result[0] == "The new avatar movie is out.", (
